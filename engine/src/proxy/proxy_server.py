...
class ProxyServer:
    ...
    async def _start_server_with_fallback(self, handler, host: str, port: int, label: str):
        """Try to start a TCP server, falling back to safer addresses on failure.

        On Windows with Python 3.13, ``getaddrinfo`` can fail for a specific
        LAN IP after urllib's blocking connections corrupt the Winsock resolver
        state.  This helper tries the configured ``host`` first, then falls back
        to ``0.0.0.0`` (all interfaces) and finally ``127.0.0.1`` (loopback)
        before giving up.  Returns the running Server object or None.
        """
        ...
        for candidate in candidates:
            try:
                srv = await asyncio.start_server(handler, candidate, port)
                if candidate != host:
                    log.warning(
                        "%s proxy: could not bind to %s:%d — fell back to %s:%d",
                        label, host, port, candidate, port,
                    )
                return srv
            except (_socket.gaierror, OSError) as exc:
                log.warning(
                    "%s proxy: bind to %s:%d failed (%s), trying next address ...",
                    label, candidate, port, exc,
                )
                last_exc = exc

        log.error(
            "%s proxy: all bind attempts failed for port %d. Last error: %s",
            label, port, last_exc,
        )
        return None
    ...
    async def start(self):
        ...
        http_srv = await self._start_server_with_fallback(
            self._on_client, self.host, self.port, "HTTP"
        )
        if http_srv is None:
            raise RuntimeError(
                f"Could not bind HTTP proxy on any address (tried {self.host!r} "
                f"and fallbacks). Check that port {self.port} is not in use and "
                f"that listen_host in config.json is a valid local IP."
            )
        socks_srv = None

        if self.socks_enabled:
            try:
                socks_srv = await self._start_server_with_fallback(
                    self._on_socks_client, self.socks_host, self.socks_port, "SOCKS5"
                )
                if socks_srv is None:
                    log.error("SOCKS5 listener failed on %s:%d: could not bind",

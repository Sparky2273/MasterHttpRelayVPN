...
class H2Transport:
    ...
    def __init__(self, connect_host: str, sni_host: str,
        ...
        # Idle keepalive: when all streams complete, immediately fire a
        # lightweight GET to this host/path to prevent Google's GFE from
        # closing the connection (~1s after last stream).  PING frames do
        # NOT reset Google's idle timer — only real HTTP streams do.
        self._keepalive_host: str | None = keepalive_host
        self._keepalive_path: str = "/robots.txt"
        self._idle_keepalive_task: asyncio.Task | None = None
    ...
    async def _close_internal(self):
        ...
        idle_task = self._idle_keepalive_task
    ...
    def _dispatch(self, event):
        ...
        if isinstance(event, h2.events.ResponseReceived):
            ...
            if state:
                state.done.set()
            # If this was the last stream, immediately start an idle keepalive.
            # Google's GFE closes H2 connections ~1 second after the last
            # stream ends, even with H2 PING frames active.  Only a real
            # HTTP stream resets its idle timer.  We fire one to /robots.txt
            # (no Apps Script execution consumed) the moment we go idle.
            remaining_streams = {sid for sid in self._streams
                                  if sid != event.stream_id}
            if not remaining_streams and self._keepalive_host and self._connected:
                if self._idle_keepalive_task is None or self._idle_keepalive_task.done():
                    self._idle_keepalive_task = asyncio.create_task(
    ...
    async def _send_idle_keepalive(self):
        """Continuously send lightweight GETs to keep H2 connection alive.

        Google's GFE closes HTTP/2 connections ~1 second after the last stream
        ends, regardless of H2 PING frames.  This loop sends GET /robots.txt
        (zero Apps Script quota, tiny response) in a tight cycle, ensuring the
        connection always has an active stream before Google's timer fires.

        Flow per cycle:
          send GET /robots.txt → response arrives (stream ends) → 0.1s pause →
          send next GET /robots.txt → ...

        The 0.1s pause is the ONLY window where the connection could be idle.
        Even with 900ms response latency from Google, the gap is safely under
        Google's ~1s close threshold.

        The loop exits when:
          - the connection drops (exception or self._connected=False)
          - the task is cancelled (_close_internal does this on disconnect)
        A new loop is started automatically via the StreamEnded → _dispatch
        trigger on the next connect/prewarm cycle.
        """
        if not self._keepalive_host:
            return

        log.debug("H2 idle keepalive loop started (host=%s)", self._keepalive_host)
        fail_count = 0
        first_ok = True
        ...
        while self._connected:
            try:
                await asyncio.wait_for(
                    self.request(
                        "GET", self._keepalive_path, self._keepalive_host,
                ...
                if first_ok:
                    first_ok = False
                    log.info("H2 idle keepalive active — connection held open via %s%s",
                             self._keepalive_host, self._keepalive_path)
                log.debug("H2 idle keepalive OK → sleeping 0.1s")
                # Brief pause between streams — well under Google's ~1s idle
                # threshold even if the previous response was slow.
                await asyncio.sleep(0.1)

            except asyncio.CancelledError:
                log.debug("H2 idle keepalive loop cancelled")
                break

            except asyncio.TimeoutError:
                fail_count += 1
                log.debug("H2 idle keepalive timeout (%d)", fail_count)
                if fail_count >= 3:
                    log.warning("H2 idle keepalive: 3 consecutive timeouts — exiting loop")
                    break
                await asyncio.sleep(0.2)

            except Exception as exc:
                log.debug("H2 idle keepalive error: %s — exiting loop", exc)
                break

        log.debug("H2 idle keepalive loop exited (connected=%s)", self._connected)

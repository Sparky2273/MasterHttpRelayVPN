...
def load_all(urls: list[str], max_age: int = _DEFAULT_MAX_AGE) -> list[str]:
    ...
    Strategy:
      ...
      • If there is NO cached copy for a URL, skip the download entirely —
        blocking network calls here corrupt the Windows Winsock resolver
        state (WinError 10054) and cause asyncio.start_server to fail with
        socket.gaierror immediately after.  The async ``refresh_all()``
        background task will fetch missing lists once the proxy is running.
    ...
    for url in urls:
        ...
        if cached is not None:
            ...
            # No local cache — skip blocking download.  The async refresh
            # task (refresh_all) will download this list in the background
            # once the event loop is running, without blocking startup.
            log.info(
                "Adblock: no cache for %s — will download in background after startup",
                url.split("/")[-1],
            )
    return all_domains
...
async def refresh_all(
    ...
    """Async background refresh.  Re-downloads missing or stale lists.

    This is the ONLY place network fetches happen.  ``load_all`` (the
    synchronous startup path) never touches the network, so this function
    also handles the first-run case where there is no local cache at all.
    ...
    for url in urls:
        ...
        # Always download if cache is missing (first-run) OR stale.
        cache_missing = not _cache_path(url).exists()
        if not cache_missing and not _cache_is_stale(url, max_age):
            cached = _read_cache(url) or []
            all_domains.extend(cached)
            continue

        action = "downloading" if cache_missing else "refreshing"
        log.info("Adblock: %s %s ...", action, url.split("/")[-1])
        text = await asyncio.to_thread(_fetch, url)
        if text:
            await asyncio.to_thread(_write_cache, url, text)
            domains = parse_hosts_text(text)
            log.info(
                "Adblock: %s — %d domains from %s",
                "downloaded" if cache_missing else "refreshed",
                len(domains),
                url.split("/")[-1],
            )
            ...
            if cache_missing:
                log.warning(
                    "Adblock: could not download %s — list inactive until next refresh",
                    url.split("/")[-1],
                )
            else:
                # Keep using stale cache rather than losing protection
                log.warning(
                    "Adblock: refresh failed for %s — keeping stale cache",
                    url.split("/")[-1],
                )
                cached = _read_cache(url) or []
                all_domains.extend(cached)
